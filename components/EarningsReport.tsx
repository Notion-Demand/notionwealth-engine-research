...
export default function EarningsReport({ payload }: EarningsReportProps) {
  ...
  async function handleExportPDF() {
    ...
    try {
      ...
      pdf.text("Quantalyze by Demandion | support@demandion.ai", margin, 6);

      // Split across pages
      let yOffset = -8;
      while (yOffset < imgH) {
        if (yOffset > -8) pdf.addPage();
        pdf.addImage(imgData, "PNG", 0, -yOffset, imgW, imgH);
        // Footer on every page
        pdf.setFontSize(7);
        pdf.setTextColor(140);
        pdf.text("Generated by Quantalyze (quantalyze.demandion.ai) | Contact: support@demandion.ai", margin, pageH - 4);
        yOffset += pageH;
      }

      const d = payload as unknown as { company_ticker?: string; quarter?: string };
      const filename = `Quantalyze_${d.company_ticker ?? "report"}_${d.quarter ?? "analysis"}_Delta.pdf`;
      pdf.save(filename);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div data-no-print className="flex items-center gap-2 pt-1">
        ...
        >
          <Download size={14} />
          {exporting ? "Exporting…" : "Quantalyze — Export Delta PDF"}
        </button>
        <button
