...
## Post-completion addendum: real cutover issues found (2026-07-17)

Executed against the real provisioned Entra tenant and the live `quantalyze.me` App Service. Several real, live bugs were found and fixed by actually testing sign-in, not just type-checking — each is a genuine deviation from the plan as originally written:

- **`UntrustedHost` on every request**: Azure App Service isn't a platform Auth.js auto-detects (unlike Vercel, via its own env var), so `trustHost: true` in `auth.ts` alone wasn't sufficient. Fixed by also setting `AUTH_TRUST_HOST=true` as an App Service environment variable — the env var is what actually makes Auth.js honor the platform's forwarded host, the code-level option alone did not.
- **`AADSTS50011` redirect URI mismatch, twice, for two different reasons**:
  1. Initially, Auth.js constructed the OAuth `redirect_uri` from the container's own internal hostname (e.g. `https://<container-id>:8080/api/auth/callback/microsoft-entra-id`) instead of `https://quantalyze.me/...` — this was the same root cause as the `UntrustedHost` issue, fixed by the same `AUTH_TRUST_HOST` env var (plus `AUTH_URL=https://quantalyze.me` set as a second, more explicit safeguard).
  2. After that fix, the error recurred because **`www.quantalyze.me` was a stale DNS record still pointing at a leftover pre-migration Vercel deployment** — the user was unknowingly testing against old Vercel-hosted code the entire time, which explains why repeated fixes on the Azure side never took effect and why no request ever appeared in Azure's logs. Root-caused by checking DNS resolution directly (`dig www.quantalyze.me`) after live-log debugging repeatedly showed zero incoming requests despite confirmed real sign-in attempts. Fixed by updating the `www` CNAME to point to `quantalyze-app.azurewebsites.net`, adding the matching `asuid.www` TXT verification record, binding `www.quantalyze.me` as a custom domain with its own managed SSL certificate, and adding a permanent `next.config.mjs` redirect from `www.quantalyze.me` to the bare domain for any user who lands there directly.
  - **Real incident during this fix**: the DNS changes were initially applied to the wrong domain (`demandion.ai`, the company's actual production domain, instead of `quantalyze.me`) due to switching between two GoDaddy DNS panels. This broke `www.demandion.ai` (also a Vercel-hosted site) until reverted. Recovered by finding the original project-specific Vercel CNAME value (`36616718852a3d5c.vercel-dns-017.com`) directly in Vercel's own dashboard (domain shown as "Invalid Configuration" still displays the expected record), restoring it, and removing the stray `asuid.www` TXT record that had been added to `demandion.ai` by mistake. Confirmed fully reverted via DNS lookups from multiple public resolvers and GoDaddy's own authoritative nameservers before proceeding with quantalyze.me's real changes.
- **Stale full-route cache bypassing the new `www` redirect**: `/login` had been cached by Next.js with a `s-maxage=31536000` (1 year) header from an unrelated `curl` test made days before this fix — full-route cache in the Next.js App Router isn't partitioned by `Host` header, so this cached response was served to `www.quantalyze.me/login` too, bypassing the redirect entirely and reintroducing the wrong-host risk. Every other page happened to be immune because they all call `getCurrentUser()` (which reads cookies, auto-opting the route out of static caching) — `/login` was the one page with no dynamic data read in its server component. Fixed with `export const dynamic = "force-dynamic"` on `app/login/page.tsx`.
- **Google Sign-In discovery**: while wiring up Google as an Entra federated identity provider, found that the `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` values already in production are placeholder values (~21/25 characters — real Google OAuth credentials are much longer, with a distinct format), meaning Google Sign-In has likely never functioned in production at all. This is unrelated to the auth migration itself but was surfaced by it. Google federation remains unconfigured in Entra pending real credentials.
- **Entra provisioning corrections**: Entra External ID has only one user flow per tenant (not separate "sign-up and sign-in" vs. "sign-in only" flow types as in the older Azure AD B2C product) — invite-only was implemented via the Graph API's `isSignUpAllowed: false` property on that single flow, set via an app-only Graph token (client-credentials flow using the Entra app registration's own credentials), since the Azure CLI's built-in first-party app had no pre-consented `IdentityProvider.ReadWrite.All`/`EventListener.ReadWrite.All`/`User.ReadWrite.All` permissions in the brand-new tenant.
- **Security note**: the Entra app registration's client secret was briefly, partially exposed in tool output during creation (a `tail -c 100` of a file that included the raw secret value on its last line). Caught immediately, the credential was revoked and regenerated before any further use — the replacement secret was never printed anywhere.
- **Status as of this addendum**: email/password sign-in, invite-only enforcement, and the `www` domain/redirect are all fixed and deployed. Full browser-based verification (Task 10, Steps 3, 5, 6, 8) and Google sign-in (Step 4) still require real interactive testing to close out — the agent executing this plan has no browser automation tool, so these steps depend on the human operator.
